...
<title>Histórico Ficha Financeira - Caixa</title>
...
if request.form<>"" then
	...
	sql="select top 1 year(dtpagto) as anocomp from corporerm.dbo.pffinanc where chapa='" & chapa & "' group by year(dtpagto) order by year(dtpagto) desc"
...
if request.form("bases")="ON" then sqlff="" else sqlff=" and e.provdescbase<>'B' "
	...
	'ff.mescomp -> month(dtpagto)
	sqla="SELECT ff.NROPERIODO, ff.CHAPA, year(dtpagto) as anocomp, e.PROVDESCBASE, ff.CODEVENTO, e.DESCRICAO, " & _
	"sum(case month(dtpagto) when 1 then ff.valor else 0 end) '1', sum(case month(dtpagto) when 2 then ff.valor else 0 end) '2', " & _
	"sum(case month(dtpagto) when 3 then ff.valor else 0 end) '3', sum(case month(dtpagto) when 4 then ff.valor else 0 end) '4', " & _
	"sum(case month(dtpagto) when 5 then ff.valor else 0 end) '5', sum(case month(dtpagto) when 6 then ff.valor else 0 end) '6', " & _
	"sum(case month(dtpagto) when 7 then ff.valor else 0 end) '7', sum(case month(dtpagto) when 8 then ff.valor else 0 end) '8', " & _
	"sum(case month(dtpagto) when 9 then ff.valor else 0 end) '9', sum(case month(dtpagto) when 10 then ff.valor else 0 end) '10', " & _
	"sum(case month(dtpagto) when 11 then ff.valor else 0 end) '11', sum(case month(dtpagto) when 12 then ff.valor else 0 end) '12' " & _
	"FROM (select * from corporerm.dbo.PFFINANC where chapa='" & chapa & "' AND year(dtpagto)=" & anoform & " union all " & _
	"select * from corporerm.dbo.PFFINANCCOMPL where chapa='" & chapa & "' AND year(dtpagto)=" & anoform & ") AS ff " & _
	"INNER JOIN corporerm.dbo.PEVENTO AS e ON ff.CODEVENTO = e.CODIGO " & _
	"WHERE ff.CHAPA='" & chapa & "' AND Year(DTPAGTO)=" & anoform & " " & sqlff & _
	"GROUP BY ff.NROPERIODO, ff.CHAPA, Year(dtpagto), e.PROVDESCBASE, ff.CODEVENTO, e.DESCRICAO " & _
	...
	sqla="SELECT ff.NROPERIODO, ff.CHAPA, year(dtpagto) as anocomp, e.PROVDESCBASE, ff.CODEVENTO, e.DESCRICAO, " & _
	"sum(case month(dtpagto) when 1 then ff.ref else 0 end) '1', sum(case month(dtpagto) when 2 then ff.ref else 0 end) '2', " & _
	"sum(case month(dtpagto) when 3 then ff.ref else 0 end) '3', sum(case month(dtpagto) when 4 then ff.ref else 0 end) '4', " & _
	"sum(case month(dtpagto) when 5 then ff.ref else 0 end) '5', sum(case month(dtpagto) when 6 then ff.ref else 0 end) '6', " & _
	"sum(case month(dtpagto) when 7 then ff.ref else 0 end) '7', sum(case month(dtpagto) when 8 then ff.ref else 0 end) '8', " & _
	"sum(case month(dtpagto) when 9 then ff.ref else 0 end) '9', sum(case month(dtpagto) when 10 then ff.ref else 0 end) '10', " & _
	"sum(case month(dtpagto) when 11 then ff.ref else 0 end) '11', sum(case month(dtpagto) when 12 then ff.ref else 0 end) '12' " & _
	"FROM (select * from corporerm.dbo.PFFINANC where chapa='" & chapa & "' AND year(dtpagto)=" & anoform & " union all " & _
	"select * from corporerm.dbo.PFFINANCCOMPL where chapa='" & chapa & "' AND year(dtpagto)=" & anoform & ") AS ff " & _
	"INNER JOIN corporerm.dbo.PEVENTO AS e ON ff.CODEVENTO = e.CODIGO " & _
	"WHERE ff.CHAPA='" & chapa & "' AND Year(DTPAGTO)=" & anoform & " " & sqlff & _
...
%>
<form method="POST" action="hstfichafc.asp" name="formff">
...
<th class=titulo colspan=28>Histórico Ficha Financeira por Caixa
<select name="anofrm" onChange="javascript:submit()">
<%
sql="select ano as anocomp from (select year(dtpagto) as ano from corporerm.dbo.pffinanc where chapa='" & chapa & "' union all " & _
"select year(dtpagto) as ano from corporerm.dbo.pffinanccompl where chapa='" & chapa & "') t group by ano order by ano desc"
rsq.Open sql, ,adOpenStatic, adLockReadOnly
