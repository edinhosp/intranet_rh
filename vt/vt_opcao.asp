...
<title>Opção Vale Transporte</title>
...
if request("codigo")<>"" or request.form<>"" then
	...
	sqla="SELECT f.NOME, f.CODSITUACAO, f.CHAPA, f.DATAADMISSAO, c.NOME AS funcao, f.CODSECAO, s.DESCRICAO AS Secao, " & _
	"p.GRAUINSTRUCAO, p.RUA, p.NUMERO, p.COMPLEMENTO, p.BAIRRO, p.CIDADE, p.CEP, p.TELEFONE1, p.TELEFONE2, p.TELEFONE3, " & _
	"p.FAX, p.SEXO, p.DTNASCIMENTO, p.EMAIL, p.CPF, p.CARTIDENTIDADE, p.UFCARTIDENT, f.PISPASEP, f.CODPESSOA, f.DATADEMISSAO " & _
	"FROM ((PFUNC AS f INNER JOIN PPESSOA AS p ON f.CODPESSOA = p.CODIGO) INNER JOIN PFUNCAO AS c ON f.CODFUNCAO = c.CODIGO) INNER JOIN PSECAO AS s ON f.CODSECAO = s.CODIGO "
...
<form method="POST" action="vt_opcao.asp">
...
if rs("codsindicato")="03" then empresa="I" else empresa="M"
select case empresa
	case "I"
...
"WHERE chapa='" & rs("chapa") & "' AND getdate() Between [ivigencia] And [fvigencia] "
...
"WHERE codigo='" & empresa & "' AND plano Not Like " & stringplano & " AND plano Not Like 'agr%' ORDER BY seq "
...
"int((now()-dtnascimento)/365.25) AS idade, c.mae " & _
"from pfdepend d, pcodparent p, pfdependcompl c " & _
