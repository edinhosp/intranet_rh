...
<title>Geração de Arquivo de Cesta-Basica</title>
...
if request.form("gerar")<>"" then
	conexao.execute "delete from ttcbasica where sessao='" & sessao & "' and opcao='V' "
...
"select '" & sessao & "', '01', null, null, 'LSUP5" & espaco2(session("usuarioname"),8) & space(11) & data1 & hora1 & space(6) & espaco2("LAYOUT-04/12/2006",57) & space(165) & "','V' "
conexao.execute sql

'registro header
sql="insert into ttcbasica (sessao,campo1,campo2,campo3,registro, opcao) " & _
"select '" & sessao & "', '02', null, null, 'TA020A" & "0175760020" & espaco2("FUND.INST.ENSINO OSASCO",24) & space(6) & data1 & dataliberacao & "C" & space(16) & mespedido & space(19) & "04" & "33" & space(48) & "SUP   " & "000001','V' "
conexao.execute sql

'registro unidade
sql="insert into ttcbasica (sessao,campo1,campo2,campo3,registro,opcao) " & _
"select '" & sessao & "', '03', '01', null, 'TA022" & espaco2("NARCISO",26) & "AV  " & espaco2("FRANZ VOEGELLI",30) & "000300" & "5.ANDAR-RH" & _
espaco2("OSASCO",25) & espaco2("VILA YARA",15) & "06020SP" & espaco2(request.form("responsavel"),20)& "190" & space(7) & "000002','V'"
conexao.execute sql

sql="insert into ttcbasica (sessao,campo1,campo2,campo3,registro, opcao) " & _
"select '" & sessao & "', '03', '03', null, 'TA022" & espaco2("VILA YARA",26) & "AV  " & espaco2("FRANZ VOEGELLI",30) & "000300" & "5.ANDAR-RH" & _
espaco2("OSASCO",25) & espaco2("VILA YARA",15) & "06020SP" & espaco2(request.form("responsavel"),20)& "190" & space(7) & "000003','V'"
conexao.execute sql

sql="insert into ttcbasica (sessao,campo1,campo2,campo3,registro, opcao) " & _
"select '" & sessao & "', '03', '04', null, 'TA022" & espaco2("JD.WILSON",26) & "AV  " & espaco2("FRANZ VOEGELLI",30) & "000300" & "5.ANDAR-RH" & _
espaco2("OSASCO",25) & espaco2("VILA YARA",15) & "06020SP" & espaco2(request.form("responsavel"),20)& "190" & space(7) & "000004','V'"
conexao.execute sql
...
"where f.codpessoa=p.codigo and f.codsecao=s.codigo and a.chapa=f.chapa collate database_default and a.sessao='" & sessao & "' " & _
...
do while not rs.eof
	...
	"00101" & valorcb & "AE" & espaco2(rs("nome"),30) & space(17) & numzero(sequencia,6) & "', " & taxacb2 & ",'V'"
...
sql="insert into ttcbasica (sessao,campo1,campo2,campo3,registro, opcao) " & _
"select '" & sessao & "', '05', null, null, 'TA029" & numzero(totalfu,8) & numzero(valort,14) & space(131) & numzero(sequencia,6) & "','V' "
conexao.execute sql

sql="select count(sessao) as totalarq from ttcbasica where sessao='" & sessao & "' and campo1 not in ('01','02','05','06') and opcao='V'"
rs.Open sql, ,adOpenStatic, adLockReadOnly
totalarq=rs("totalarq")
rs.close

sql="insert into ttcbasica (sessao,campo1,campo2,campo3,registro,opcao) " & _
"select '" & sessao & "', '06', null, null, 'LSUP9" & "00000002" & "00000002" & numzero(totalarq,8) & space(277) & "','V' "
conexao.execute sql
	
sql="select * from ttcbasica where sessao='" & sessao & "' and opcao='V' order by campo1, right(campo2,8)"
end if
%>

<p class=titulo>Geração de arquivo para Cesta Básica</p>
<%
if request.form("gerar")="" then
...
if request("acao")="excluir" then
	...
	sql="delete from ttcbasica_sel where chapa='" & chapa & "' and sessao='" & sessao & "' "
	conexao.execute sql
	manutencaocb=1
end if
...
if request.form("incluir")<>"" then
	if request.form("novachapa")<>"" then
		...
		"select '" & sessao & "', '" & request.form("novachapa") & "','V' "
		conexao.execute sql2
	end if
	manutencaocb=1
end if
...
if request.form("alterar")="" then
	...
	if manutencaocb<>1 then
		sql="DELETE FROM ttcbasica_sel where sessao='" & sessao & "' "
		conexao.execute sql
		...
		"WHERE f.CODSITUACAO Not In ('I','D') AND f.CODSINDICATO<>'03' AND f.CODTIPO='N' and c.opcbasica='V' order by f.chapa, codsecao "
		'response.write sql
		rs.Open sql, ,adOpenStatic, adLockReadOnly
		...
		do while not rs.eof
			...
			if salcomp<=limite then
				...
				"select '" & sessao & "', '" & rs("chapa") & "', " & nraccess(rs("saltotal")) & "," & nraccess(salcomp) & ", 'V'"
				conexao.execute sql2
				'response.write "7"
			end if
		rs.movenext
...
"where f.chapa collate database_default=c.chapa and f.codsecao=s.codigo and c.sessao='" & sessao & "' and c.opcao='V' order by f.nome "
rs.Open sql, ,adOpenStatic, adLockReadOnly
%>
<form method="POST" action="cb_arquivo.asp">
...
<tr>
	...
	<td class=campo align="center">&nbsp;
		<a href="cb_arquivo.asp?acao=excluir&chapa=<%=rs("chapa")%>">
...
<%
sql="select valor from iParametros where parametro='cbcartao'"
rs2.open sql, ,adOpenStatic, adLockReadOnly:valorcb=rs2("valor"):rs2.close
sql="select valor from iParametros where parametro='cbtaxa'"
rs2.open sql, ,adOpenStatic, adLockReadOnly:taxacb=rs2("valor"):rs2.close
valorcb=replace(valorcb,".",",")
taxacb=replace(taxacb,".",",")
%>

<p>	Data Liberação: <input type="text" size="9" name="dataliberacao" value="<%=dataliberacao%>"><br>
Valor do Benefício: <input type="text" size=5 name="valorcb" value="<%=valorcb%>">
Taxa por cartão:  <input type="text" size=5 name="taxacb" value="<%=taxacb%>"><br>
...
"FROM corporerm.dbo.PFUNC AS f LEFT JOIN ttcbasica_sel2 AS a ON f.CHAPA collate database_default= a.CHAPA " & _
"inner join corporerm.dbo.pfcompl c on c.chapa=f.chapa " & _
"WHERE datademissao between getdate()-30 and getdate() and Not f.CODSINDICATO='03' AND f.CODTIPO='N' and c.opcbasica='V' "
rs.Open sql, ,adOpenStatic, adLockReadOnly
%>
<table border="1" cellpadding="0" cellspacing="1" style="border-collapse: collapse">
...
'----------------------------------------
sql="select * from ttcbasica where sessao='" & sessao & "' and opcao='V' order by campo1, campo2, substring(registro,159,6) "
rs.Open sql, ,adOpenStatic, adLockReadOnly
totalf=0:totalg=0
rs.movefirst
