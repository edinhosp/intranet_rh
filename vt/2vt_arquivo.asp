...
SQL="SELECT F.CHAPA, F.NOME, D.NOME AS MAE FROM corporerm_teste.dbo.PFUNC AS F LEFT JOIN corporerm_teste.dbo.PFDEPEND AS D ON F.CHAPA = D.CHAPA " & _
...
'response.write "6"
	...
	'sql="DELETE CHAPA FROM ttvtchapa WHERE chapa in (" & request.form("excluir") & ")":conexao.execute sql

	sql="SELECT f.CODTIPO, unidade=case when codtipo='T' then '90' else left(codsecao,2) end, f.CODSECAO, s.DESCRICAO " & _
"FROM corporerm_teste.dbo.PFUNC f INNER JOIN corporerm_teste.dbo.PSECAO s ON f.CODSECAO=s.CODIGO " & _
"WHERE f.DIASUTPROXMES>0 AND f.CODSITUACAO<>'D' " & _
"AND f.CHAPA collate database_default IN (SELECT CHAPA FROM TTVTCHAPA2 GROUP BY CHAPA) " & _
...
"FROM corporerm_teste.dbo.PFUNC F, corporerm_teste.dbo.PPESSOA P, QRY_MAE M " & _
"WHERE F.DIASUTPROXMES>0 AND F.CODSITUACAO<>'D' and f.codpessoa=p.codigo and f.chapa=m.chapa " & _
"AND F.CHAPA collate database_default IN (SELECT CHAPA FROM TTVTCHAPA2 GROUP BY CHAPA) " & _
...
response.write "" & rs("chapa")
	...
	elseif left(endereco,3)="AV" and left(endereco,3)<>"AV." and left(endereco,7)<>"AVENIDA" then
		endereco=replace(endereco,"AV ","AV   ")
	...
	espaco2(replace(rs("mae"),"'"," "),30) & espaco2(endereco,45) & numzero(textopuro(rs("numero"),2),6) & espaco2(complemento,15) & _
...
"FROM ttvtchapa2 INNER JOIN (corporerm_teste.dbo.PFUNC f INNER JOIN (((corporerm_teste.dbo.PFVALETR fv INNER JOIN corporerm_teste.dbo.PVALETR v ON fv.CODLINHA=v.CODIGO) INNER JOIN corporerm_teste.dbo.PTARIFA t ON v.CODTARIFA=t.CODIGO) INNER JOIN sPTARIFA s ON t.CODIGO collate database_default=s.CODIGO) ON f.CHAPA collate database_default=fv.CHAPA) ON (ttvtchapa2.CODLINHA=fv.CODLINHA collate database_default) AND (ttvtchapa2.CHAPA=fv.CHAPA collate database_default) " & _
...
data1 & space(1) & dataini & datafim & "00000000" & perctaxa & "000000115.11" & "N" & "0001" & "AP%  R$ " & "' "
conexao.execute sql
...
%>
<form method="POST" action="vt_arquivo.asp">
...
sql="INSERT INTO ttvtchapa ( CHAPA ) SELECT CHAPA FROM corporerm_teste.dbo.PFVALETR WHERE DTFIM>'" & dtaccess(datainicio) & "' GROUP BY CHAPA":response.write sql&"<br>":conexao.execute sql
sql="INSERT INTO ttvtchapa2 ( CHAPA, CODLINHA ) SELECT CHAPA, CODLINHA FROM corporerm_teste.dbo.PFVALETR WHERE DTFIM>'" & dtaccess(datainicio) & "' GROUP BY CHAPA, CODLINHA":response.write sql:conexao.execute sql
...
"FROM (SELECT campo1, campo3 FROM ttvtransporte WHERE sessao='" & sessao & "' and campo1='06') fu " & _
"LEFT JOIN (SELECT campo1, campo3 FROM ttvtransporte WHERE sessao='" & sessao & "' and campo1='07') it ON fu.campo3 = it.campo3 " & _
...
"FROM (SELECT campo1, campo3 FROM ttvtransporte WHERE sessao='" & sessao & "' and campo1='06') fu " & _
"RIGHT JOIN (SELECT campo1, campo3 FROM ttvtransporte WHERE sessao='" & sessao & "' and campo1='07') it ON fu.campo3 = it.campo3 " & _
