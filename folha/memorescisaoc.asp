...
<form method="POST" action="memorescisaoc.asp" name="form">
...
sql2="select dtpagtorescisao=dtpagto , recibos=count(chapa) from (select distinct dtpagto, chapa from corporerm.dbo.PFFINANC where DTPAGTO>=GETDATE()-90 and NROPERIODO in (10) ) z group by dtpagto"
...
"inner join (select distinct chapa, dtpagto, nroperiodo from corporerm.dbo.pffinanc where dtpagto='" & dtaccess(request.form("dtpagto")) & "' and nroperiodo=10) p " & _
"on p.chapa=f.chapa " & _
"where f.chapa>'0' " & selecao
...
<tr>
	...
	<b>ASSUNTO:</b><br>
	<input type="text" class="form_input" style="text-align:left;font-size:14px" size="80" name=obs value="Pagamento de Rescisões Complementares <%=monthname(month(dtpagto+request.form("diaspag")))%>/<%=year(dtpagto+request.form("diaspag"))%>">
	</td>
</tr>
...
Solicitamos os valores, conforme abaixo discriminados, para pagamento em <b><%=dtpagto%></b> das Rescisões Contratuais do mês de <%=monthname(month(dtpagto+request.form("diaspag")))%>/<%=year(dtpagto+request.form("diaspag"))%>:
<br>
...
<br>
<%=textoconsignado%>
<input type="text" class="form_input" style="text-align:left" size="80" name=obs value=".">
<br>
<br>
...
Osasco,&nbsp;<%=dia & " de " & monthname(month(now())) & " de " & year(now()) %><bR><br><br>
Ao<br>
