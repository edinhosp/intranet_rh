...
<title>Carta para Pagamento de INSS</title>
...
%>
<p class=titulo>Emissão de Autorização de Pagtº - INSS&nbsp;<%=titulo %>
<form method="POST" action="creditoinss.asp" name="form">
<table border="0" width="250" cellspacing="0"cellpadding="3">
<tr>
	<td class=titulo>Competência do INSS</td>
</tr>
<tr>
...
&nbsp;
<%
sql2="select anocomp, mescomp from corporerm.dbo.pffinanc where dtpagto>getdate()-60 group by anocomp, mescomp "
...
%>
	<option value="<%=comp%>" <%=temp1%>><%=comp%>&nbsp;</option>
<%
rs.movenext:loop
rs.close
%>
...
mescomp=left(temp,2)
anocomp=right(temp,4)
sessao=session.sessionid
data=dateserial(anocomp,mescomp+1,1)
datam=dateserial(anocomp,mescomp-1,1)
mescompm=month(datam)
datapg=dateserial(anocomp,mescomp+1,2)
'	rs.Open sqlb, ,adOpenStatic, adLockReadOnly

'**** grupos mes
sql1="SELECT '" & sessao & "' as sessao, g.Ordem, g.Grupo, g.dia, g.CODIGO, f.CODSINDICATO, ff.ANOCOMP, ff.MESCOMP, Sum(VALOR*fator) AS custo, Left(CODSECAO,2) AS Campus " & _
...
"AND g.excecao='0' and g.ordem=6 " & _
...
sql2="UNION ALL "
sql3="SELECT '" & sessao & "', g.Ordem, g.Grupo, g.dia, g.CODIGO, f.CODSINDICATO, ff.ANOCOMP, ff.MESCOMP, Sum(VALOR*fator) AS custo, Left(CODSECAO,2) AS Campus " & _
"FROM grupo_custo g, corporerm.dbo.pffinanc ff, corporerm.dbo.PFUNC f WHERE ff.CHAPA=f.CHAPA AND g.CODIGO=ff.CODEVENTO collate database_default " & _
"AND g.excecao='1' and g.ordem=6 " & _
"GROUP BY g.Ordem, g.Grupo, g.dia, g.CODIGO, f.CODSINDICATO, ff.ANOCOMP, ff.MESCOMP, Left(CODSECAO,2) " & _
"HAVING ff.ANOCOMP=" & anocomp & " AND ff.MESCOMP=" & mescompm & " "
sql4=sql1 & sql2 & sql3
sql5="SELECT GRUPO, SUM(CUSTO) AS VALOR FROM (" & sql4 & ") t GROUP BY GRUPO"

data1=datapg
sqld="select diaferiado from corporerm.dbo.gferiado " & _
"where diaferiado='" & dtaccess(datapg) & "' "
...
referente ao recolhimento do <b>INSS</b> da Folha de Pagamento competência <b><%=monthname(mescomp)&"/"&anocomp%></b>, 
a ser recolhido no dia <b><%=datapg%></b>.
<br>
<br>
<br>
