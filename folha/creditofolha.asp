...
&nbsp;
<%
sql2="select anocomp, mescomp, nroperiodo from corporerm.dbo.pffinanc where dtpagto>getdate()-60 group by anocomp, mescomp, nroperiodo order by anocomp, mescomp "
rs.Open sql2, ,adOpenStatic, adLockReadOnly
rs.movefirst:do while not rs.eof
codhtml=numzero(rs("mescomp"),2)&rs("anocomp")&rs("nroperiodo")
'if dtpagto=rs("dtpagto") then temp1="selected" else temp1=""
comp=numzero(rs("mescomp"),2)&"/"&rs("anocomp")
%>
	<option value="<%=codhtml%>" <%=temp1%>><%=comp%>&nbsp;(Período: <%=rs("nroperiodo")%>)</option>
<%
rs.movenext:loop
rs.close
%>
...
if nroperiodo="3" then tipofunc="estagiário" else tipofunc="funcionário"
'response.write anocomp & "<br>" & mescomp & "<br>" & nroperiodo & "<br>" & dtpagto
sql0="select top 1 anocomp, mescomp, nroperiodo, dtpagto, count(chapa) from corporerm.dbo.pffinanc " & _
"where anocomp=" & anocomp & " and mescomp=" & mescomp & " and nroperiodo=" & nroperiodo & " group by anocomp, mescomp, nroperiodo, dtpagto order by count(chapa) desc"
rs.Open sql0, ,adOpenStatic, adLockReadOnly
if rs.recordcount>0 then dtpagto=rs("dtpagto") else dtpagto=now()
rs.close
...
Agência 2856 - Ag.Cidade de Deus<br>
Osasco - SP<br>
<br>
<br>
...
corrente<%=frase%> do<%=frase%>&nbsp;<%=quant%>&nbsp;<%=tipofunc%><%=frase%> conforme relação anexa e disquete, para crédito em <b><%=dtpagto%></b>.
<br>
<br>
<br>
