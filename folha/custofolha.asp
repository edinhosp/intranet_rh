...
%>
<p class=titulo>Emissão de Custo da Folha Pag.&nbsp;<%=titulo %>
<form method="POST" action="custofolha.asp" name="form">
...
sql1="delete from grupo_custo_temp where sessao='" & sessao & "' "
conexao.execute sql1
sql1="delete from grupo_custo_table where sessao='" & sessao & "' "
conexao.execute sql1

'**** grupos mes
sql2="INSERT INTO grupo_custo_temp (Sessao, Ordem, Grupo, dia, codevento, codsin, ANOCOMP, MESCOMP, custo, Campus ) " & _
...
"FROM grupo_custo g, corporerm.dbo.pffinanc ff, corporerm.dbo.PFUNC f WHERE ff.CHAPA=f.CHAPA and g.CODIGO=ff.CODEVENTO collate database_default " & _
"AND g.excecao='0' " & _
...
datair=data+dias+4
datair=dateserial(year(data),month(data)+1,20)
...
"FROM corporerm.dbo.PFPERFF ff, corporerm.dbo.PFUNC f WHERE ff.CHAPA=f.CHAPA AND f.CODTIPO='N' " & _
"and ff.nroperiodo in (" & periodo & ") " & _
...
'***** pis
datapis=dateserial(anocomp,mescomp+1,25)
...
"SELECT '" & sessao & "', 11, 'PIS', 25, '" & dtaccess(datapis) & "', f.CODSINDICATO, ff.ANOCOMP, ff.MESCOMP, " & _
...
"FROM corporerm.dbo.pffinanc AS f, corporerm.dbo.pevento AS e, corporerm.dbo.pfunc AS pf " & _
"WHERE f.CODEVENTO=e.codigo AND pf.CHAPA=f.chapa AND e.PROVDESCBASE In ('P','D') " & _
"and f.nroperiodo in (" & periodo & ") " & _
"GROUP BY pf.CODTIPO, Left(CODSECAO,2), pf.CODSINDICATO, f.ANOCOMP, f.MESCOMP, f.dtpagto " & _
"HAVING f.ANOCOMP=" & anocomp & " AND f.MESCOMP=" & mescomp & " " & _
...
rs.close
sql7b="update grupo_custo_temp set data=#" & dtaccess(datacerta) & "# where ordem=13 and sessao='" & sessao & "' "
...
"FROM corporerm.dbo.pfperfer_old pf, corporerm.dbo.pfferias_old fer, corporerm.dbo.PEVENTO e, corporerm.dbo.PFUNC f WHERE pf.chapa=f.chapa AND pf.nroperiodo=fer.nroperiodo AND pf.dtvencimento=fer.dtvencimento AND pf.chapa=fer.chapa AND fer.codevento=e.codigo " & _
...
"FROM corporerm.dbo.PFUFERIASPER pf, corporerm.dbo.PFUFERIASVERBAS fer, corporerm.dbo.PEVENTO e, corporerm.dbo.PFUNC f " & _
...
sql10="update grupo_custo_temp set data=dateadd(m,1,cast(year(data) as char)+'/'+cast(month(data) as char)+'/20') where sessao='" & sessao & "' and ordem=15 "
