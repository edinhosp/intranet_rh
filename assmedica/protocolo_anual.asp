...
if request.form="" then
	...
	"FROM assmed_mudanca am inner join assmed_beneficiario ab on am.chapa=ab.CHAPA inner join corporerm.dbo.pfunc f on f.chapa collate database_default=ab.chapa " & _
	"where am.empresa='U' and getdate() between am.ivigencia and am.fvigencia "
	conexao.execute sql
	sql="INSERT INTO ttamprotocolo ( sessao,chapa,Funcionario,Beneficiario,empresa,plano,codigo,compr,id,tabela,parentesco) " & _
	...
	"WHERE adm.empresa='U' and getdate() between adm.ivigencia and adm.fvigencia "
	conexao.execute sql
...
%>
<form name="protocolo" action="protocolo_anual.asp" method="post">
...
<%
		...
		for a=1 to vez
			...
			if emitir="ON" then
				if tabela="tit" then
					sql="UPDATE assmed_mudanca SET anual = 1 WHERE id_mudanca=" & id 
					conexao.execute sql
				end if
				if tabela="dep" then
					sql="UPDATE assmed_dep_mudanca SET anual = 1 WHERE id_mud=" & id 
					conexao.execute sql
				end if
