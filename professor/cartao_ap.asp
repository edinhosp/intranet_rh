...
sqln="select top 20 chapa from corporerm.dbo.pfunc where codsindicato='03' and codsituacao in ('A','F') " & _
" and chapa collate database_default in (select chapa from quem_nomeacoes) " & _
"order by chapa "
sqln="select top 50 chapa from corporerm.dbo.pfunc where codsindicato='03' and codsituacao in ('A','F') " & _
" and chapa>'02406' " & _
"order by chapa "
sqln="select top 50 chapa from corporerm.dbo.pfunc where codsindicato='03' and codsituacao in ('A','F','Z') " & _
" and chapa collate database_default in (select chapa from grades_rt where codevento in ('275') and fim>getdate() ) " & _
"order by chapa "
sqln="select top 5 chapa from corporerm.dbo.pfunc where codsindicato='03' and codsituacao in ('A','F','Z') " & _
" and chapa collate database_default in (select chapa from grades_rt where codevento in ('128','255','256','257','258','034') ) " & _
"order by chapa "
...
<tr><td class=campo>Espelho de Marcação de Ponto</td><td class=campo align="right"><%=rsn.absoluteposition%></td></tr></table>
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" width="600">
<tr><td class=campo colspan=2>FUNDAÇÃO INSTITUTO DE ENSINO PARA OSASCO</td></tr></table>
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" width="600">
<tr><td class=campo colspan=2>Período: <%=mes & "/" & ano %></td></tr></table>
<hr>
...
<table border="1" bordercolor="gray" cellpadding="0" cellspacing="0" style="border-collapse: collapse" width="650">
  <tr>
...
redim htrab(31)
'dim nat(31,6)
	sqlcr="select chapa, data, batida from corporerm.dbo.abatfun where chapa='" & chapa & "' and data='" & dtaccess(dtponto) & "' order by batida"
	sqlcr="select chapa, day(data) as dia, data, batida, natureza, status from corporerm.dbo.abatfun where chapa='" & chapa & "' and month(data)=" & mes & " and year(data)=" & ano & " "
...
for t=1 to udia
	...
	sqlh="select htrab from corporerm.dbo.aafhtfun where chapa='" & chapa & "' and data='" & dtaccess(datah) & "' "
...
<%
sqln="select chapa1, perlet, curso, materia, diasem, inicio, termino, min(horini) as horini, max(horfim) as horfim, count(codtur) as aulas " & _
"from grades_5ch " & _
...
"group by chapa1, perlet, curso, materia, diasem, inicio, termino "
rs.Open sqln, ,adOpenStatic, adLockReadOnly
if rs.recordcount>0 then
rs.movefirst
do while not rs.eof
...
<tr>
	...
	<td class="campor"><%=weekdayname(rs("diasem"))%></td>
...
<%
	...
	response.write "<tr><td class="campor" colspan=8>Sem aulas</td></tr>"
end if
rs.close
%>
</table>
