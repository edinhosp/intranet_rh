...
<form method="POST" action="cartao_periodot.asp">
...
<%
	...
	'rt
	sqld="select f.chapa, f.nome, c.nome as funcao, s.descricao as setor from corporerm.dbo.pfunc f, corporerm.dbo.psecao s, corporerm.dbo.pfuncao c where f.codsecao=s.codigo and f.codfuncao=c.codigo and f.codsituacao in ('A','F','Z','E') and f.chapa collate database_default in (select chapa from corporerm.dbo.pfsalcmp where codevento in ('255','256','257','258','138','128')) order by f.chapa "
	'todos
	sqld="select f.chapa, f.nome, c.nome as funcao, s.descricao as setor from corporerm.dbo.pfunc f, corporerm.dbo.psecao s, corporerm.dbo.pfuncao c where f.codsecao=s.codigo and f.codfuncao=c.codigo and f.codsituacao in ('A','F','Z','E') and f.chapa collate database_default in (select chapa from corporerm.dbo.pfunc where codsindicato='03' and left(codsecao,2)='03' and codtipo='N' ) order by f.nome "
...
<%
sqln="select chapa1, perlet, coddoc curso, materia, diasem, inicio, termino, min(horini) as horini, max(horfim) as horfim, count(ta) as aulas " & _
"from g5ch " & _
...
<tr>
	...
	<td class="campor"><%=rs("diasem")%></td>
	<td class="campor"><%=rs("horini") & "-" & rs("horfim")%></td>
