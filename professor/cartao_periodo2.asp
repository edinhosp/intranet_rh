...
<form method="POST" action="cartao_periodo2.asp">
...
<tr><td colspan=3 class=titulo>
	<input type="radio" name="qual" value="1">Outros
	<input type="radio" name="qual" value="2">RT
	<input type="radio" name="qual" value="3">RHT
	<input type="radio" name="qual" value="4">Todos - (RT/RHT)
</td>
...
<%
	...
	select case qual
		case 1 'outros
			sqld="select f.chapa, f.nome, c.nome as funcao, s.descricao as setor from corporerm.dbo.pfunc f, corporerm.dbo.psecao s, corporerm.dbo.pfuncao c where f.codsecao=s.codigo and f.codfuncao=c.codigo " & _
			"and f.chapa in ('00075','00214','00272','00287','00300','00301','00453','00772','02185','02507','02726','02759','02826','02831','02665','02972') "
			sqld="select f.chapa, f.nome, c.nome as funcao, s.descricao as setor from corporerm.dbo.pfunc f, corporerm.dbo.psecao s, corporerm.dbo.pfuncao c where f.codsecao=s.codigo and f.codfuncao=c.codigo " & _
			"and f.chapa collate database_default in (select chapa1 from achapa) "
		case 2 'rt
			sqld="select f.chapa, f.nome, c.nome as funcao, s.descricao as setor from corporerm.dbo.pfunc f, corporerm.dbo.psecao s, corporerm.dbo.pfuncao c where f.codsecao=s.codigo and f.codfuncao=c.codigo and f.codsituacao in ('A','F','Z','E') " & _
			"and (f.chapa collate database_default in (select chapa from corporerm.dbo.pfsalcmp where codevento in ('255','256','257','258','138','128')) " & _
			"or f.chapa collate database_default in (select chapa from corporerm.dbo.pfunc where chapa in ('01164','01165')) ) " & _
			"order by f.chapa"
		case 3 'rht
			sqld="select f.chapa, f.nome, c.nome as funcao, s.descricao as setor from corporerm.dbo.pfunc f, corporerm.dbo.psecao s, corporerm.dbo.pfuncao c where f.codsecao=s.codigo and f.codfuncao=c.codigo and f.chapa collate database_default in (select chapa from quem_nomeacoes where tipo='RHT') order by f.chapa"
		case 4 'todos -rht/rt
			sqld="select top 100 f.chapa, f.nome, c.nome as funcao, s.descricao as setor " & _
			...
			"where f.codsituacao in ('A','F','Z','E') and f.CODTIPO='N' and f.CODSINDICATO='03' " & _
			"and ( f.chapa collate database_default not in (select chapa from quem_nomeacoes where tipo='RHT') " & _
			"and f.chapa collate database_default not in (select chapa from corporerm.dbo.pfsalcmp where codevento in ('255','256','257','258','138','128')) " & _
			"and f.chapa collate database_default not in (select chapa from corporerm.dbo.pfunc where chapa in ('01164','01165')) ) " & _
			"and f.chapa>'00000' " & _
			"order by f.chapa"
	end select
...
<tr><td class="campor"><%=rsd("chapa")%></td>
...
Redim marc(diasloop,6), formato(diasloop,6), htrab(diasloop)
'dim nat(31,6)
for e=data1 to data2
	idmatriz=e-(data1)
	'marcações do chronus
	sqlcr="select chapa, day(data) as dia, data, batida, natureza, status from corporerm.dbo.abatfunam where chapa='" & rsd("chapa") & "' " & _
	"and data='" & dtaccess(e) & "' " & _
	"UNION ALL " & _
	"select chapa, day(data) as dia, data, batida, natureza, status from corporerm.dbo.abatfun where chapa='" & rsd("chapa") & "' " & _
	"and data='" & dtaccess(e) & "' "
...
'for t=1 to udia
	datah=t
	sqlh="select htrab from corporerm.dbo.aafhtfunam where chapa='" & rsd("chapa") & "' and data='" & dtaccess(datah) & "' " & _
	"UNION ALL " & _
	"select htrab from corporerm.dbo.aafhtfun where chapa='" & rsd("chapa") & "' and data='" & dtaccess(datah) & "' "
...
for e=data1 to data2
	...
	"where chapa='" & rsd("chapa") & "' and dia_mes='" & dtaccess(dtponto) & "' "
	'grade horaria
...
"AND i.CHAPA='" & rsd("chapa") & "' AND ('" & dtaccess(dateserial(ano,mes,1)) & "' Between mand_ini And mand_fim " & _
...
<%
sqln="select chapa1, perlet, coddoc curso, materia, /*diasem,*/ inicio, termino, sum(ta) as aulas " & _
"from g5ch " & _
"where chapa1='" & rsd("chapa") & "' " & _
...
"group by chapa1, perlet, coddoc, materia, /*diasem,*/ inicio, termino "
rs.Open sqln, ,adOpenStatic, adLockReadOnly
if rs.recordcount>0 then
rs.movefirst
do while not rs.eof
