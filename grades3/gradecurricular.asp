...
<%
sqla="SELECT coddoc, curso, enfase from grades_3 where coddoc in (select coddoc from grades_user where usuario='" & session("usuariomaster") & "') GROUP BY coddoc, curso, enfase order by curso "
rs.Open sqla, ,adOpenStatic, adLockReadOnly
rs.movefirst:do while not rs.eof 
%>
	<option <%if request.form("coddoc")=rs("enfase") & rs("coddoc") then response.write "selected "%> value="<%=rs("enfase")&rs("coddoc")%>"><%=rs("curso")%> (<%=rs("coddoc")%>) <%if rs("enfase")<>"NOR" then response.write " " & rs("enfase")%></option>
<%
rs.movenext:loop
rs.close
%>  
...
<%
sqla="select perlet2 from grades_3 group by perlet2 order by perlet2 desc"
rs.Open sqla, ,adOpenStatic, adLockReadOnly
rs.movefirst:do while not rs.eof 
%>
...
inicio=now()
	...
	"AND perlet2='" & datae & "' AND per.coddoc='" & codcur & "' and per.enfase='" & enfase & "' " & _
...
<%
	...
	"FROM grades_3ch AS g, (select chapa, nome from pfunc union all select chapa, nome from grades_novos) AS p  " & _
...
<%
	sql2="SELECT serie, turma, codtur, turno=round((case when turno<10 then turno*10 else turno end)/10,0), sum(g.ta) AS aulas FROM grades_3chi AS g " & _
...
<%
	...
	"FROM grades_3ch AS g, grades_aux_prof AS p " & _
