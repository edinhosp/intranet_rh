...
<tr><td class=titulo><select size="1" name="codcur" onChange="javascript:submit()">
	...
	<option <%if codcur="EFA" then response.write "selected "%> value="EFA">-> Todos os cursos na mesma página <- </option>
<%
sqla="SELECT coddoc, curso from grades_3 where coddoc in (select coddoc from grades_user where usuario='" & session("usuariomaster") & "') GROUP BY coddoc, curso order by curso "
...
sqla="SELECT gch.perlet, gch.perlet2, gch.perlet3, gch.coddoc, gch.curso, gp.GC, gch.turno, gch.serie, gch.turma, " & _
...
"FROM ((grades_3chi AS gch INNER JOIN grades_per AS gp ON (gch.coddoc=gp.coddoc) AND (gch.perlet = gp.perlet) AND (gch.perlet2 = gp.perlet2)) " & _
"INNER JOIN grades_aux_prof AS f ON gch.chapa1 = f.CHAPA) INNER JOIN grades_gc ON (gp.perlet = grades_gc.perlet) AND (gch.serie = grades_gc.serie) AND (gp.GC = grades_gc.GC) AND (gp.coddoc = grades_gc.coddoc) " & _
"WHERE gch.coddoc='" & codcur & "' AND gch.diasem=DATEPART (dw,'" & dtaccess(dataponto) & "') " & _
...
if codcur="EFA" then
	...
	"FROM ((grades_3chi AS gch INNER JOIN grades_per AS gp ON (gch.coddoc = gp.coddoc) AND (gch.perlet = gp.perlet) AND (gch.perlet2 = gp.perlet2)) " & _
	"INNER JOIN grades_aux_prof AS f ON gch.chapa1 = f.CHAPA) INNER JOIN grades_gc ON (gp.perlet = grades_gc.perlet) AND (gch.serie = grades_gc.serie) AND (gp.GC = grades_gc.GC) AND (gp.coddoc = grades_gc.coddoc) " & _
	"WHERE gch.diasem=DATEPART (dw,'" & dtaccess(dataponto) & "') " & _
	"AND '" & dtaccess(dataponto) & "' Between inicio And termino AND '" & dtaccess(dataponto) & "' Between pini And pfim "
end if
if request.form("ordem")="turma" then sqla=sqla & "ORDER BY gch.turno, gp.GC, gch.serie, gch.turma, a5,a3,a1 "
if request.form("ordem")="nome" then sqla=sqla & "ORDER BY gch.turno, gp.GC, f.nome "
'response.write sqla
...
if rs("turno")="72" then turnod="Vespertino"
if rs("turno")="73" then turnod="Noturno"
if rs("turno")="74" then turnod="Noturno" 'alfa
...
if codcur="EFA" then textocurso="DOS CURSOS LIVRES" else textocurso="DO CURSO LIVRE DE " & rs("curso")
%>
<!-- borda -->
...
<tr>
	<td class="campop" align="left"><b>PONTO DO PESSOAL DOCENTE <%=textocurso%></b></td>
...
if lastperlet<>rs("perlet")&rs("gC")&rs("turno") then
if rs("turno")="1"  then turnod="Matutino"
if rs("turno")="71" then turnod="Matutino"
if rs("turno")="2"  then turnod="Vespertino"
if rs("turno")="72" then turnod="Vespertino"
...
%>
<tr>
	<td class=campo colspan=3 height=<%=altura%>><font size=2><b><%=rs("curso")%></b></font> - <%=turnod%></td>
<%
rsc.movefirst
...
rsc.close
%>
	<td class="campor" colspan=2 height=<%=altura%>>&nbsp;Período Letivo: <%=rs("perlet")%></td>
</tr>
<%
end if
...
<%
linha=linha+1
lastperlet=rs("perlet")&rs("gC")&rs("turno")
rs.movenext
loop
...
sql="select coddoc from grades_dias where coddoc='" & icodcur & "' and turno=" & iturno & " and data='" & dtaccess(idata) & "' "
rs2.Open sql, ,adOpenStatic, adLockReadOnly
existe=rs2.recordcount:rs2.close
if existe=0 then
	sql="INSERT INTO grades_dias (coddoc, temp, turno, data, dialetivo, pagina) SELECT '" & icodcur & "','" & icurso & "'," & iturno & ",'" & dtaccess(idata) & "'," & idl & "," & ipg & " "
	conexao.execute sql
else
	sql="UPDATE grades_dias SET pagina=" & ipg & ",dialetivo=" & idl & " WHERE data='" & dtaccess(idata) & "' AND turno=" & iturno & " AND coddoc='" & icodcur & "'"
	conexao.execute sql
end if ' existe
