...
if request.form("B2")<>"" then
	...
	if request.form("localizar")="" then
		...
		session("loc370")=request.form("localizar")
	end if
	'if session("loc424")<>"" then
		if isnumeric(session("loc370")) then
			session("sql370d")="where (codigo like '%" & session("loc370") & "%') "
		else
			session("sql370d")="where (nome like '%" & session("loc370") & "%') "
		end if
	'else
		'session("sqld424")=""
	'end if
...
sqlat="SELECT CODIGO, NOME, TIPO, STATUS FROM (select chapa codigo, nome, tipo=case when codtipo='T' then 'ESTAGIÁRIO' when codsindicato='03' then 'PROFESSOR' else 'ADMINISTRATIVO' end, s.descricao as status from corporerm.dbo.pfunc f, corporerm.dbo.pcodsituacao s where (f.chapa<'10000' or f.chapa>'90000') and f.codsituacao=s.codcliente " & _
"UNION ALL " & _
...
sqlaf="SELECT CODIGO, NOME, TIPO, STATUS FROM (select chapa codigo, nome, tipo=case when codtipo='T' then 'ESTAGIÁRIO' when codsindicato='03' then 'PROFESSOR' else 'ADMINISTRATIVO' end, s.descricao as status from corporerm.dbo.pfunc f, corporerm.dbo.pcodsituacao s where (f.chapa<'10000' or f.chapa>'90000') and f.codsituacao=s.codcliente ) AS QUERY " 
sqlaa="SELECT CODIGO, NOME, TIPO, STATUS FROM (SELECT E.MATRICULA codigo, E.NOME, 'ALUNO' tipo, S.DESCRICAO status FROM corporerm.dbo.EALUNOS E LEFT JOIN corporerm.dbo.USITMAT S ON E.STATUS=S.CODSITMAT) AS QUERY " 
...
sql1=sqla & sqlb & session("sql370d") & sqlc

if session("a370")="E" then sql1="SELECT E.MATRICULA as codigo, E.NOME, 'ALUNO' as tipo, S.DESCRICAO as status FROM corporerm.dbo.EALUNOS E LEFT JOIN corporerm.dbo.USITMAT S ON E.STATUS=S.CODSITMAT " & session("sql370d") & " order by nome "
...
if Session("PrimeiraVez")<>"Nao" then
	...
	rs.CacheSize = registros
	rs.PageSize = registros
	set rs.ActiveConnection = conexao ':response.write sql1
	rs.Open sql1, ,adOpenStatic, adLockReadOnly
...
End Sub
%>
<form method="POST" action="pessoas.asp" name="form">
<input type="hidden" name="vez1" value="<%=session("PrimeiraVez")%>">
<p class=titulo style="margin-top: 0; margin-bottom: 0">Ouvidoria - UNIFIEO</p>
<p style="margin-top: 0; margin-bottom: 0"><font color="blue">
Localizar por nome/chapa: <input type="text" name="localizar" size=35 value="<%=session("loc370")%>">
...
response.write "<a href=""pessoas.asp?folha=" & 1 & chr(34) & "><img src='../images/setafirst1.gif' border='0'></a>"
response.write "<a href=""pessoas.asp?folha=" & atual-1 & chr(34) & "><img src='../images/setaprevious1.gif' border='0'></a>"
end if
...
response.write "<a href=""pessoas.asp?folha=" & atual+1 & chr(34) & "><img src='../images/setanext1.gif' border='0'></a>"
response.write "<a href=""pessoas.asp?folha=" & rs.pagecount & chr(34) & "><img src='../images/setalast1.gif' border='0'></a>"
end if
%>
	</td>
...
<%
if request.form("tipo")="" then seltipo="T" else seltipo=request.form("tipo")

%>

<table border="0" width="650" cellspacing="0" style="border-collapse: collapse" cellpadding="0">
<tr>
    <td class=campo width="100%" valign="center" align="left">Tipo: 
	<input type="radio" name="tipo" value="T" <%if seltipo="T" then response.write "checked"%> onclick="javascript:submit();"> Todos
	<input type="radio" name="tipo" value="F" <%if seltipo="F" then response.write "checked"%> onclick="javascript:submit();"> Funcionários
	<input type="radio" name="tipo" value="A" <%if seltipo="A" then response.write "checked"%> onclick="javascript:submit();"> Alunos
	</td>
  </tr>
</table>
