...
<form method="POST" action="trct2012.asp" name="form">
<%
...
tamtemp=len(rs("nome")):if tamtemp>46 then tamtemp=46
%>
	<option  value="<%=rs("chapa")%>" <%=temps%> ><%=rs("nome") & string(46-tamtemp," ") & " - " & rs("datademissao")%></option>
<%
rs.movenext:loop
rs.close
%>
...
<br>
Percentual de pensão alimentícia: <input type="text" name="pensao" size="4" value="0%"> | Pensão FGTS: <input type="text" name="pensaofgts" size="4" value="0%">
<br>
...
tpquit=dateadd("yyyy",1,rs("admissao"))
if rs("demissao")+30>=tpquit then termo="H" else termo="Q"
...
sql21="select top 1 dtmudanca from corporerm.dbo.pfhstsal where chapa='" & ch(b) & "' and dtmudanca<='" & dtaccess(calculo) & "' and salario>0 order by dtmudanca desc"
sql2="select salario from corporerm.dbo.pfhstsal where chapa='" & ch(b) & "' and dtmudanca in (" & sql21 & ") and salario>0"
rs2.Open sql2, ,adOpenStatic, adLockReadOnly
salario=rs2("salario")
rs2.close
%>
...
<tr><td class=rcampo2 style="border:1px solid">&nbsp;21 Tipo de Contrato<br>&nbsp;<span class=campo1><%=tipocontrato%></td></tr>
<tr><td class=rcampo2 style="border-right:1px solid;border-left:1px solid">&nbsp;22 Causa do Afastamento<br>&nbsp;<span class=campo1><%=desc_%></td></tr>
</table>

<table border="0" bordercolor="#000000" cellpadding="0" cellspacing="0" style="border-collapse: collapse;" width="650">
<tr>
	<td class=rcampo1 style="border:1px solid">&nbsp;23 Remuneração Mês Anterior<br>&nbsp;<span class=campo1><%=formatnumber(salario,2)%></td>
	<td class=rcampo1 style="border:1px solid">&nbsp;24 Data de Admissão<br>&nbsp;<span class=campo1><%=dataform(rs("admissao"))%></td>
	<td class=rcampo1 style="border:1px solid">&nbsp;25 Data do Aviso Prévio<br>&nbsp;<span class=campo1><%=dataform(rs("dtavisoprevio"))%></td>
	<td class=rcampo1 style="border:1px solid">&nbsp;26 Data de afastamento<br>&nbsp;<span class=campo1><%=dataform(rs("demissao"))%></td>
	<td class=rcampo1 style="border:1px solid">&nbsp;27 Cód. Afastamento<br>&nbsp;<span class=campo1><%=cod_%></td>
</tr>
</table>

<table border="0" bordercolor="#000000" cellpadding="0" cellspacing="0" style="border-collapse: collapse;" width="650">
<tr>
	<td class=rcampo1 style="border-left:1px solid;border-right:1px solid">&nbsp;28 Pensão Alimentícia (%)<br>&nbsp;<span class=campo1><%=request.form("pensao")%><!--<input type="text" class="form_input10" value="" size="4">--></td>
	<td class=rcampo1 style="border-left:1px solid;border-right:1px solid">&nbsp;29 Pensão Alimentícia (%) (FGTS)<br>&nbsp;<span class=campo1><%=request.form("pensaofgts")%><!--<input type="text" class="form_input10" value="" size="4">--></td>
	<td class=rcampo1 style="border-left:1px solid;border-right:1px solid">&nbsp;30 Categoria do Trabalhador<br>&nbsp;<span class=campo1><%=numzero(rs("codcategoria"),2)%></td>
</tr>
</table>
<table border="0" bordercolor="#000000" cellpadding="0" cellspacing="0" style="border-collapse: collapse;" width="650">
<tr>
	<td class=rcampo1 style="border:1px solid">&nbsp;31 Código Sindical<br>&nbsp;<span class=campo1><%=cod_sind%></td>
	<td class=rcampo1 style="border:1px solid">&nbsp;32 CNPJ e Nome da Entidade Sindical Laboral<br>&nbsp;<span class=campo1><%=cnpj_sind & " <span style='font-size:10pt'>" & nome_sind & "</span>"%></td>
</tr>
...
"from corporerm.dbo.pffinanc f " & _
"	inner join corporerm.dbo.pevento e on e.codigo=f.codevento " & _
...
if ulttipo<>rs2("agrupar") and rs2.absoluteposition>1 then 
	...
	if contaprov<colsprov then
		linhatemp=(colsprov-contaprov)/3-3
...
if rs2("rubrica")="66" and valor>0 then
	sqlf="select dtvencferias, saldoferias from corporerm.dbo.pfunc where chapa='" & rs("chapa") & "' "
...
if totalizou=0 then 
	...
	if contaded<colsded then
		linhatemp=int((colsded-contaded)/3)
		for l=1 to linhatemp
			response.write "<tr><td class=rcampo3></td><td class=rcampo4></td><td class=rcampo3></td><td class=rcampo4></td><td class=rcampo3></td><td class=rcampo4></td></tr>"
		next
	end if
...
<tr>
	...
	<td class=rcampo1>&nbsp;29 Pensão Alimentícia (%) (FGTS)<br>&nbsp;<span class=campo1><%=request.form("pensaofgts")%><!--<input type="text" class="form_input10" value="" size="4">--></td>
</tr>
</table>
