...
<form method="POST" action="pontov2.asp" name="form">
...
if codcur="POSN"  then codcur1="not in ('ART','PIN','PCL','MPS','QTE','PHO','PCI','FDF','MDI','DOP') "
%>
	...
	if linha=diasemana then
		...
		if edia(ultimo)=1 or linha=1 then 'é feriado
			...
			if dateserial(anoagora,mesagora,ultimo)<=int(now+diasimpr) then
				response.write "<a href='pontov2.asp?d=" & ultimo & "&m=" & mesagora & "&a=" & anoagora & "&c=" & codcur & "&o=" & opcao  & "&or=" & ordem & "' class=r>"
			end if
			if cint(ultimo)=cint(diaagora) then response.write "<b>"
			response.write ultimo
			response.write "</a>"
...
if codcur="POSN"  then codcur1=" not in ('ART','PIN','PCL','MPS','QTE','PHO','PCI','FDF','MDI','DOP') ":tpcurso="P "
...
if tpcurso="P " or tpcurso="P" or tpcurso="G" and session("usuariomaster")="02379" then
'*************** inicio teste **********************
...
<tr>
	...
	<td class=titulo align="center" rowspan=1>Turma<br>Início aulas</td>
	<td class=titulo align="center" rowspan=1 colspan=4>Horário / Assinatura</td>
...
%>
<tr>
	<td class="campor" height=35>&nbsp;1</td>
...
response.write "	<td class=titulo align=""center"" rowspan=1>Turma<br>Início aulas</td>"
response.write "	<td class=titulo align=""center"" rowspan=1 colspan=4 >Horário / Assinatura</td>"
'response.write "	<td class=titulo align=""center"" colspan=" & classes & ">Classes</td>"
if left(tpcurso,1)<>"G" then
	response.write "<td align=""center"" style=""font-size:6pt; font-family:tahoma;font-weight:normal;background-color:Silver;color:Black;"">#Aulas</td>"
end if
response.write "</tr>"
linha=4+1
...
if left(tpcurso,1)="G" then colunas=7 else colunas=8
%>
<tr>
	<td class=campo colspan=<%=colunas%> height=<%=altura%>>Período Letivo: <%=rs("perlet")%> - <%=turno%></td>
<%
if left(tpcurso,1)="G" then
...
<%end if%>
	...
	<td class="campor" nowrap><%=rs("codtur")%>
<%	
	inicio="":termino=""
	inicios=""
	for i=10 to rs.fields.count-2
		if inicios<>"" and rs.fields(i)="X" then inicios=inicios & "|"
		if rs.fields(i)="X" then inicios=inicios & rs.fields(i).name
	next
	response.write "<Br>" & inicios
%>	
	</td>
	<td class="campop" width=50 align="center"><b>:</b></td>
	<td class="campop" width=75>&nbsp;</td>
	<td class="campop" width=50 align="center"><b>:</b></td>
	<td class="campop" width=75>&nbsp;</td>
<%
if left(tpcurso,1)="G" then
...
<%for a=1 to 6
	if a=2 or a=4 then txtcampo="<b>:</b>" else txtcampo="&nbsp;"
%>
	<td class=campo align="center"><%=txtcampo%></td>
<%next%>
</tr>
<%
rs1.movenext
