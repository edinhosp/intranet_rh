...
if session("a31")<>"T" then response.write "<script language='JavaScript' type='text/javascript'>self.close();</script>"
%>
<html>
<head>
...
if request.form("a1")="" and request.form("a2")="" and request.form("a3")="" and request.form("a4")="" and request.form("a5")="" and request.form("a6")="" then
	tudook=0:response.write "<script language='JavaScript' type='text/javascript'>alert('Selecione os horários de aula!');</script>"
		...
		sql="UPDATE grades_2 SET "
		sql=sql & "perlet    ='" & perlet & "', "
		sql=sql & "perlet2   ='" & perlet2 & "', "
		sql=sql & "perletsg  ='" & perlet & "', "
	...
	if request.form("bt_excluir")<>"" then
		...
		sql="UPDATE grades_2 set deletada=1, usuarioa='" & session("usuariomaster") & "', dataa=getdate() WHERE id_grade=" & session("id_alt_grade")
		if session("usuariomaster")="02379" then sql="UPDATE grades_2 set deletada=1 WHERE id_grade=" & session("id_alt_grade")
...
sqla="SELECT codcur, curso as nome FROM grades_per where lanc" & lanc & _
"and codcur in (select codcur from grades_user where usuario='" & session("usuariomaster") & "') " & _
"GROUP BY codcur, curso ORDER BY curso "
sqla="SELECT p.coddoc, g.curso as nome FROM grades_per p, g2cursoeve g where g.coddoc=p.coddoc and p.tper='G' and p.lanc " & lanc & _
"and p.coddoc in (select coddoc from grades_user where usuario='" & session("usuariomaster") & "') " & _
"GROUP BY p.coddoc, g.curso ORDER BY g.curso "
...
<%
sqla="SELECT codturno, descturno from eturnos where codturno in (1,2,3,5,61,62) "
if session("usuariomaster")="02379" then sqla="SELECT codturno, descturno from eturnos where codturno in (1,2,3,5,61,62,12) "
rsd.Open sqla, ,adOpenStatic, adLockReadOnly
if rsd.recordcount>0 then
response.write "<option value='0'>....</option>"
rsd.movefirst:do while not rsd.eof
...
<%
'sqla="SELECT grupo from grades_per where codcur=" & codcur & " and perlet='" & perlet & "' order by serie "
sqla="SELECT p.grupo from grades_per p, grades_gc g where g.coddoc=p.coddoc and g.perlet=p.perlet and p.gc=g.gc " & _
" and p.coddoc='" & codcur & "' and p.perlet='" & perlet & "' and g.serie=" & serie & " group by p.grupo order by grupo "
rsd.Open sqla, ,adOpenStatic, adLockReadOnly
if rsd.recordcount>0 then
'response.write "<option value='0'>..</option>"
rsd.movefirst:do while not rsd.eof
...
<%
sqla="SELECT p.gc from grades_per p, grades_gc g where g.coddoc=p.coddoc and g.perlet=p.perlet and p.gc=g.gc " & _
" and p.coddoc='" & codcur & "' and p.perlet='" & perlet & "' and g.serie=" & serie & " "
rsd.Open sqla, ,adOpenStatic, adLockReadOnly
if rsd.recordcount>0 then gc=rsd("gc") else gc="NOR"
rsd.close
%>
...
<%
sqlc="select count(chapa1) as taulas from g2ch where chapa1='" & chapa1 & "' and perlet2 like '" & left(perlet2,4) & "%" & right(perlet2,1) & "' and deletada=0 and juntar=0 AND ATIVO=1"
...
%>
	...
	<td class=fundo align="left">
<%
sqlc="select count(chapa1) as taulas from g2ch where chapa1='" & chapa2 & "' and perlet2 like '" & left(perlet2,4) & "%" & right(perlet2,1) & "' and deletada=0 and juntar=0 AND ATIVO=1"
rsd.Open sqlc, ,adOpenStatic, adLockReadOnly
taulas2=rsd("taulas")
rsd.close
%>	
...
<tr>
	<td class=titulo align="center">
  	<% if (taulas1>limitech and (chapa1<>"99999" and chapa1<>"99998")) or (taulas2>limitech and (chapa2<>"99999" and chapa2<>"99998")) then %>
