...
<%
sql2="select perlet from g2turmas where coddoc in (select coddoc from grades_user where usuario='02379') group by perlet order by perlet desc"
...
sql1="select t.*, c.tipocurso, c.hcoord, inicio, termino, lancamento from g2turmas t, g2cursos c, g2periodoaula p where p.perlet=t.perlet and t.codcur=c.codcur and t.codper=c.codper and " & _
...
if rs.recordcount>0 then 
	...
	hcoord=rs("hcoord")
	if isnull(rs("maxcur")) then alunosturma=rs("fimapr") else alunosturma=rs("maxcur")
	...
	semturma=0
	if aberto<>0 and periodolanc<now() then aberto2=1 else aberto2=0
else
	semturma=1
end if
if session("usuariomaster")="02379" then response.write id_grdturma
rs.close
...
for b=2 to 7
	...
	if codhor>0 then
		...
		"from g2aulas g inner join corporerm.dbo.umaterias m on g.codmat=m.codmat collate database_default " & _ 
		"where g.id_grdturma=" & id_grdturma & " and g.codhor=" & codhor & " and g.deletada=0 " & _
		...
		if rs2.eof then 
			if aberto<>0 or session("master")=1 then 
				...
				texto=texto & "<div align=""center"" id=""0"" alt=""1""><hr>Aula Vaga</div>"
			end if
			classe="campoa"
		end if
	'<img src="../images/espelho.jpg" width="16" height="16" border="0" alt="">
		do while not rs2.eof
			imagem="":nome1="":nome2="":hordisp=1
			...
			if isnull(rs2("chapa1"))=false then
				'-----------
				if left(perlet,4)="2014" or left(perlet,4)="2013" then
				sql91="select chapa from g2disp where chapa='" & rs2("chapa1") & "' and codhor=" & codhor
				rs3.Open sql91, ,adOpenStatic, adLockReadOnly
				if rs3.recordcount=0 then hordisp=0 else hordisp=1
				rs3.close
				end if
				'-----------
			...
			if isnull(rs2("chapa2"))=false then
				'-----------
				if left(perlet,4)="2014" then
				sql92="select chapa from g2disp where chapa='" & rs2("chapa2") & "' and codhor=" & codhor
				rs3.Open sql92, ,adOpenStatic, adLockReadOnly
				if rs3.recordcount=0 then hordisp=0
				rs3.close
				end if
				'-----------
			...
			if hordisp=0 then
				estilodisp="""border:2px dashed red"""
			else
				estilodisp="""border:0px dashed"""
			end if
			'if periodolanc<now() and session("usuariomaster")<>"02379" then
			if (periodolanc<int(now())+1 or aberto=0) and session("master")<>1 then
				texto=texto & "<div style=" & estilodisp & " id=""" & rs2("id_grdaula") & """ alt=""1"" >"
			else
				texto=texto & "<div style=" & estilodisp & " id=""" & rs2("id_grdaula") & """ alt=""1"" onclick=""NewWindow('gradenovaaula.asp?idaula=" & rs2("id_grdaula") & "&idturma=" & id_grdturma & "&codhor=" & codhor  & "','InclusaoGradev2','650','400','yes','center');return false"" onfocus=""this.blur()"">"
			end if
		...
		texto="<hr><div id=""0"" alt=""0""></div>"
		classe="fundo"
	end if
%>
	<td class="<%=classe%>" valign="top" nowrap width="150">
	<%=texto%>
	</td>
<%
next
...
"from g2aulas a inner join (select id_grdaula, data, id_data from g2aulasdata where deletada=0 group by id_Grdaula, data, id_data) d " & _
"on a.id_grdaula=d.id_grdaula inner join corporerm.dbo.umaterias m on a.codmat=m.codmat collate database_default " & _
"left join grades_aux_prof f on f.chapa=a.chapa1 " & _
"where a.id_grdturma=" & id_grdturma & " and d.data between '" & dtaccess(inicioperiodo) & "' and '" & dtaccess(terminoperiodo) & "' " & _
...
if tipocurso="2" then sql5="SELECT g.CODMAT, m.MATERIA, g.NAULASSEM, g.CARGAHORARIA, z.total, g.tipo " & _
"FROM ((select codcur, codper, grade, periodo, codmat, naulassem, cargahoraria, tipo='O' from corporerm.dbo.ugrade union select codcur, codper, grade, periodo, codmat, naulassem, cargahoraria, tipo from g2_ugrade where tipo='G') g INNER JOIN corporerm.dbo.umaterias m ON g.codmat=m.codmat) " & _
...
if tipocurso<>"2" then sql5="SELECT g.CODMAT, m.MATERIA, g.NAULASSEM, g.CARGAHORARIA, z.total, tipo='' " & _
...
taulasem=taulasem+teste2
if rs2("tipo")="G" then cssfundo="campoa" else cssfundo="campol"
%>
<tr>
	<td class="campop"><%if tipocurso<>"2" then %><a href="gradenovapos.asp?idturma=<%=id_grdturma%>&codmat=<%=rs2("codmat")%>" onclick="NewWindow(this.href,'InclusaoGradev2pos','650','600','yes','center');return false" onfocus="this.blur()">
	<img src="../images/mais.gif" width="10" height="10" border="0" alt=""></a>
<% else response.write "&nbsp;" end if%>	</td>
	<td class="<%=cssfundo%>"><div name="<%=rs2("materia")%>"><b><%=rs2("materia")%></b> (<font color="gray"><%=rs2("codmat")%></font>)</div></td>
	<td class="<%=cssfundo%>" align="center"><%=rs2("naulassem")%></td>
	<td class="<%=cssfundo%>" align="center"><%=rs2("cargahoraria")%></td>
	<td class="<%=cssfundo%>" align="center" style="border-right:2px solid 000000"><%=corfont4%><%=rs2("total")%></font></td>
	<td class="campor">
<%
...
if rs3("status")="85" or rs3("status")="90" or rs3("status")="23" then imprimestatus=1 else imprimestatus=0
	if rs3("descricao")="CURSANDO" or rs3("descricao")="CURSANDO REP." or rs3("descricao")="APROVADO" then 
...
if rs3("descricao")="CURSANDO" or rs3("descricao")="CURSANDO REP." or rs3("descricao")="APROVADO" then imprimestatus=0
	corfonte1="<font color=""black"">":corfonte2="<font color=""black"">":final3=""
'	if session("usuariomaster")="02379" and (rs3("descricao")<>"CURSANDO" and rs3("descricao")<>"APROVADO" and rs3("status")<>"85") then
	if session("usuariomaster")="02379" and (rs3("descricao")<>"CURSANDO" and rs3("descricao")<>"CURSANDO REP." and rs3("descricao")<>"APROVADO") or imprimestatus=1 then
		response.write corfonte1 & rs3("descricao") & ": " & rs3("alunos") & "</font>"
	end if
...
<tr>
	<td colspan="2">&nbsp;</td>
	<td class="campo" align="center"><%=taulasem%></td>
	<Td colspan="3">&nbsp;</td>
</tr>
<tr>
...
do while not rs2.eof
	...
	response.write " -> "
	response.write "</font><font color=""blue"">"
	response.write rs2("alunos")
	total1=total1+rs2("alunos")
	response.write "</font>"
	if rs2.absoluteposition=rs2.recordcount then response.write "<font color=""green""> = " & total1 & "</font>" else response.write "<br>"
rs2.movenext
loop
rs2.close
'end if
...
rs2.close
if isnull(initurma) or initurma="" then initurma=1:if codcur="ADM" then initurma=6
if isnull(fimturma) or fimturma="" then fimturma=1:if codcur="ADM" then fimturma=8
'response.write "<br>" & initurma
'response.write "<br>" & fimturma
'response.write "<br>" & turmaid

if turmaid<>"" and fimturma>=1 then 
...
if tipocurso="2" or tipocurso="12" then
sqlcusto="select custo=sum(custo), adic=sum(adicionais), enc=sum(enc_mes), prov=sum(provisao), lancadas=sum(ta) " & _
"from ( " & _
"select g.chapa1, g.coddoc, ta, valoraula=(case when valoraula is null then 37.37 else valoraula end), " & _
"custo=round(ta*4.5*(case when valoraula is null then 37.37 else valoraula end),2), " & _
"adicionais=round((ta*4.5*(case when valoraula is null then 37.37 else valoraula end))*0.225,2), " & _
"enc_mes=round((ta*4.5*(case when valoraula is null then 37.37 else valoraula end)*1.225)*0.09,2), " & _
"provisao=round((ta*4.5*(case when valoraula is null then 37.37 else valoraula end)*1.225)*0.2119,2) " & _
"from g2ch g " & _
...
if isnull(rs2("lancadas")) then lancadas=0 else lancadas=rs2("lancadas")
if lancadas<taulasem then faltal=taulasem-lancadas else faltal=0
if semturma=1 then
	custo=taulasem*4.5*66.67
	adic=custo*0.225
	enc=(custo*1.225)*0.09
	prov=custo*1.225*0.2119
	...
	custo2=faltal*4.5*66.67:custo=custo+custo2
	adic2=custo2*0.225:adic=adic+adic2
	enc2=custo2*1.225*0.09:enc=enc+enc2
	prov2=custo2*1.225*0.2119:prov=prov+prov2
	custototal=custo+adic+enc+prov '+custo2+adic2+enc2+prov2
end if
rs2.close
coordenacao=hcoord*4.5*66.34
sqla="select alunoscurso=sum(case when maxcur is null then fimapr else maxcur end) from g2turmas t inner join g2periodoaula a on a.perlet=t.perlet and '" & dtaccess(inicioperiodo) & "' between inicio and termino and coddoc='" & coddoc & "'"
rs2.Open sqla, ,adOpenStatic, adLockReadOnly
alunoscurso=rs2("alunoscurso")
rs2.close
coordenacao=(coordenacao / alunoscurso) * alunosturma
if coordenacao="" or isnull(coordenacao) then coordenacao=0
orientacao=3*4*51.86
custoaluno=alunosturma * 217.44 '180.00
custototal=custototal+coordenacao+orientacao+custoaluno
...
if custototal>0 then
	if mensalidade>0 then pef=int(custototal/mensalidade)+1 else pef=0
	pei=int(pef/((100-evasao)/100))+1
else
	pef=0:pei=0
end if
fatormens=0.75
receita=mensalidade*fatormens*alunosturma

%>
<table border="0" bordercolor="#000000" cellpadding="2" cellspacing="0" style="border-collapse: collapse" >
<tr>
	<td class=fundo style="border:1px solid" colspan=4 align="center"><b>Custo da Turma</b>
...
<tr><td class=campo colspan=2 style="border:1px solid">Orientações Estagio/TCC <span style="font-size:8px;vertical-align:top">(11)</span></td>
	<td class=campo colspan=2 style="border:1px solid" align="right"><%=formatnumber(orientacao,2)%></td></tr>
<tr><td class=campo colspan=2 style="border:1px solid">Coordenação (<%=alunosturma%>/<%=alunoscurso%>) <span style="font-size:8px;vertical-align:top">(10)</span></td>
	<td class=campo colspan=2 style="border:1px solid" align="right"><%=formatnumber(coordenacao,2)%></td></tr>
<tr><td class=campo colspan=2 style="border:1px solid">Custo por Aluno <span style="font-size:8px;vertical-align:top">(12)</span></td>
	<td class=campo colspan=2 style="border:1px solid" align="right"><%=formatnumber(custoaluno,2)%></td></tr>
...
<%if session("usuariomaster")="02379" or session("master")=1 or session("usuariogrupo")="COORD.CURSO" then%>
	<tr><td class=campo colspan=2 style="border:1px solid">Receita (<%=alunosturma &" x " & mensalidade%>) <span style="font-size:8px;vertical-align:top">(13)</span></td>
	<td class=campo colspan=2 style="border:1px solid" align="right"><%=formatnumber(receita,2)%></td></tr>
<%end if%>
...
<tr>
	...
	<br>(10) Rateio do custo de coordenação do curso entre todos os alunos <br>do curso pelo nº de alunos da turma.
	<br>(11) Média de 3 horas destinadas para orientações de estágio,<br>TCC e atividades.
	<br>(12) Custo apurado contabilmente referente as despesas <br>administrativas (manutenção, equipamentos, consumo etc.).
</td>
</tr>
