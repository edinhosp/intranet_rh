...
"FROM g2ch AS gr INNER JOIN g2cursoeve AS gc ON gc.coddoc = gr.coddoc " & _
...
if finaliza=1 then
	...
	if dt_abertura="" then dt_abertura="01/01/" & year(now())
	if dt_encerramento="" then dt_encerramento="31/12/" & year(now())
