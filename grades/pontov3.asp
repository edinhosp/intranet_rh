...
<form method="POST" action="pontov3.asp" name="form">
	...
	if linha=diasemana then
		...
		if edia(ultimo)=1 or linha=1 then 'é feriado
			...
			if dateserial(anoagora,mesagora,ultimo)<=int(now+diasimpr) then
				response.write "<a href='pontov3.asp?d=" & ultimo & "&m=" & mesagora & "&a=" & anoagora & "&c=" & codcur & "&o=" & opcao  & "&or=" & ordem & "' class=r>"
			end if
			if cint(ultimo)=cint(diaagora) then response.write "<b>"
			response.write ultimo
			response.write "</a>"
...
<tr>
	...
	<td class=titulo align="center" rowspan=1 colspan=3>Horário / Assinatura</td>
...
response.write "	<td class=titulo align=""center"" rowspan=1 colspan=3 >Horário / Assinatura</td>"
