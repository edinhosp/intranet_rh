...
<%
sqla="SELECT coddoc, curso from grades_2 where coddoc in (select coddoc from grades_user where usuario='" & session("usuariomaster") & "') GROUP BY coddoc, curso order by curso "
...
<tr>
	<td class=titulo>Grade Curricular vigente em: <input type="text" class=a name="dataemissao" size="12" maxlength="12" value=<%=formatdatetime(dataemissao,2)%>>
	</td>
</tr>
</table>
...
inicio=now()
	...
	datae=request.form("dataemissao")
	periodos=0
	...
	"mat.NAULASSEM, mat.CARGAHORARIA " & _
	...
	"AND '" & dtaccess(datae) & "' Between [pini] And [pfim] AND per.coddoc='" & codcur & "' " & _
	"group by per.coddoc, per.codcur, per.curso, per.perlet, per.perlet2, per.perletsg, per.pini, per.pfim, gc.GC, gc.serie, mat.CODMAT, mat.MATERIA, mat.NAULASSEM, mat.CARGAHORARIA " & _	
...
<%
	...
	"FROM g2ch AS g, grades_aux_prof AS p  " & _
	...
	"FROM grades_gc AS gc INNER JOIN grades_materias AS gm ON (gc.serie=gm.serie) AND (gc.GC=gm.GC) AND (gc.coddoc=gm.coddoc) " & _
...
<%
	sql2="SELECT serie, turma, codtur, codtursg,  turno=round((case when turno<10 then turno*10 else turno end)/10,0), sum(g.ta) AS aulas FROM g2ch AS g " & _
	...
	"AND '" & dtaccess(datae) & "' Between [INICIO] And [TERMINO] " & _
	"GROUP BY codtur, round((case when turno<10 then turno*10 else turno end)/10,0), turma, serie, codtursg "
	'response.write sql2
...
<%
	...
	"AND '" & dtaccess(datae) & "' Between [INICIO] And [TERMINO] and g.deletada=0 and g.ativo=1 " & _
...
<%
	perlet=lastperlet
	perle2=lastperlet2
	sql4="SELECT g.perlet, g.coddoc, g.turno, g.serie, g.turma, g.diasem, g.descricao, g.codmat, g.materia, g.chapa1, p.nome, g.usuarioa, g.usuarioc, g.id_grade " & _
	"FROM g2ch AS g, grades_aux_prof AS p  " & _
	"WHERE g.chapa1=p.chapa and g.perlet='" & perlet & "' and g.perlet2='" & perlet2 & "' AND g.coddoc='" & codcur & "' " & _
	"and g.codmat not in (SELECT gm.CODMAT " & _
	"FROM grades_gc AS gc INNER JOIN grades_materias AS gm ON (gc.serie=gm.serie) AND (gc.GC=gm.GC) AND (gc.coddoc=gm.coddoc) " & _
