...
<%
sqla="select curso, coddoc from grades_5 group by curso, coddoc "
...
<tr>
	<td class=titulo><select size="1" name="perlet">
<%
sqla="select perlet2 from grades_5ch group by perlet2 order by perlet2 desc"
rs.Open sqla, ,adOpenStatic, adLockReadOnly
...
if finaliza=1 then
	...
	perlet2=request.form("perlet")
	'database=request.form("database")
	'datarel=dtaccess(database)

	sql1="SELECT coddoc, curso, perlet, perletsg FROM grades_5 WHERE deletada=0 " & _
	"AND perlet2='" & perlet2 & "' " & _
	"GROUP BY coddoc, curso, perlet, perletsg " & _
	"HAVING coddoc='" & codcur & "' ORDER BY perlet "
	...
	do while not rs.eof
		...
		curso=rs("curso")
		perletsg=rs("perletsg")
	rs.movenext
	numero=numero+1
	loop
	rs.close
	sqla="select codcur from grades_per where coddoc='" & codcur & "' and perlet2='" & perlet2 & "' "
	rs.Open sqla, ,adOpenStatic, adLockReadOnly
	if rs.recordcount>0 then codcursg=rs("codcur") else codcursg=codcur
	rs.close
	
	for a=0 to ubound(periodoletivo)
		numero=0
		sql2="SELECT turno, descturno FROM grades_5 g, eturnos e WHERE deletada=0 and g.turno=e.codturno " & _
		"AND coddoc='" & codcur & "' AND perlet Like '" & periodoletivo(a) & "' " & _
		...
		for b=0 to ubound(turnograde)
			numero=0
			sql5="SELECT serie, turma, cast([serie] as varchar)+[turma] AS STURMA FROM grades_5 WHERE deletada=0 " & _
			"AND coddoc='" & codcur & "' AND perlet Like '" & periodoletivo(a) & "%' " & _
...
<tr>
	<td class=grupo colspan="<%=3+ubound(sturma)+1%>"><b><%=curso%></b> - em <%=formatdatetime(now,2)%></td>
</tr>
<tr>
...
<%
			numero=0
			sql3="SELECT diasem FROM grades_5 WHERE deletada=0 " & _
			"AND coddoc='" & codcur & "' AND perlet Like '" & periodoletivo(a) & "' AND turno=" & turnograde(b) & " " & _
...
'linha=linha+ubound(horaini)
				...
				'for d=1 to 6
					...
					for e=0 to ubound(sturma)
						...
						sql6="select codmat, materia, chapa1, chapa2, nome, codsala, juntar, jturma, dividir, codtur, inicio, termino FROM grades_5, " & _
						"grades_aux_prof as f WHERE deletada=0 AND chapa1=chapa " & _
						" AND coddoc='" & codcur & "' AND perlet='" & perlet & "'" & " AND turno=" & turno & _
						" AND serie=" & serie & "" & " AND turma='" & turma & "'" & " AND diasem=" & diasem & _
						" AND a" & d+1 & "=1 and ativo=1 order by inicio "
						...
						do while not rs.eof
							...
							chapa2=rs("chapa2")
							codtur=rs("codtur")
							inicio=rs("inicio"):termino=rs("termino")
							'*******alunos******
	'if codcur=260 then
	'	if perlet="2004/0" and turma="B" then perlet="2003/2"
	'end if
	'sql3="SELECT UMATALUN.STATUS, USITMAT_1.DESCRICAO, Count(UMATRICPL.MATALUNO) AS alunos " & _
	'antes do having "GROUP BY UMATALUN.STATUS, USITMAT_1.DESCRICAO " & _
	...
	'"HAVING UMATALUN.STATUS In ('01') "
	'"AND UMATALUN.CODMAT='" & codmat & "' AND UALUCURSO.STATUS='01' " & _
	...
	if rs("juntar")=-1 then
		obs="<font color=blue>Junta turma " & rs("jturma") & "<br><font color=black>"
	elseif rs("dividir")=-1 then
		obs="<font color=red>Divide turma" & "<br><font color=black>"
	else
		obs=""
	end if
	compl(d,e)=alunos & sala & obs
							celula1=celula1 & "<b>" & materia & "</b> (entre " & inicio & " e " & termino & ") " & "</b><br>" & nome & " (" & chapa1 & ")" & "<br>"
...
<%
			...
			next 'diasemana c
	sql0="SELECT CURSO, diretor, coordenador, adjunto, chefedepto, secretaria FROM grades_per " & _
	"WHERE CODdoc='" & codcur & "' and perlet='" & periodoletivo(a) & "' "
