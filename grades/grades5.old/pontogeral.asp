...
<tr><td class=titulo><select size="1" name="codcur" onChange="javascript:submit()">
	...
	<option value="1" <%if codcur=1 then response.write "selected"%>>Narciso</option>
	<option value="3" <%if codcur=3 then response.write "selected"%>>Vila Yara</option>
	</select></td>
<%
if codcur="" then codcur1="='0'":codcur="0"
if codcur=3 then codcur1="in ('ART','PIN','PCL','MPS')"
if codcur=1 then codcur1="not in ('ART','PIN','PCL','MPS')"
%>	
<td class=titulo><select size="1" name="opcao" onChange="javascript:submit()">
<%
sqla="SELECT tipo from grades_5chi where coddoc " & codcur1 & " GROUP BY tipo order by tipo "
	...
	if linha=diasemana then
		...
		if edia(ultimo)=1 or linha=1 then 'é feriado
			...
			if dateserial(anoagora,mesagora,ultimo)<=int(now+10) then
				response.write "<a href='pontogeral.asp?d=" & ultimo & "&m=" & mesagora & "&a=" & anoagora & "&c=" & codcur & "&o=" & opcao  & "&or=" & ordem & "' class=r>"
			end if
			if cint(ultimo)=cint(diaagora) then response.write "<b>"
			response.write ultimo
			response.write "</a>"
...
if finaliza=1 then
	...
	opcao=request.form("opcao")  'tipo CLT/Convidados
	periodoletivo=ano & "/"
...
"WHERE gch.coddoc " & codcur1 & " AND gch.diasem=DATEPART (dw,'" & dtaccess(dataponto) & "') and gch.tipo='" & opcao & "' " & _
"AND '" & dtaccess(dataponto) & "' Between [inicio] And [termino] AND '" & dtaccess(dataponto) & "' Between [pini] And [pfim] "
if request.form("ordem")="turma" then sqla=sqla & "ORDER BY gch.coddoc, gch.turno, gch.serie, gch.turma, a5,a3,a1 "
if request.form("ordem")="nome" then sqla=sqla & "ORDER BY gch.coddoc, gch.turno, f.nome "
'response.write sqla
...
<tr>
	<td class="campop" align="left"><b><%=titulo%>DOS CURSOS DA PÓS-GRADUAÇÃO</b></td>
...
response.write "<tr>"
response.write "	<td class="campop" align="left"><b>" & titulo & " DOS CURSOS DA PÓS-GRADUAÇÃO</b></td>"
...
response.write "	<td class="campop" align="right" nowrap>&nbsp;<!--" & "º dia letivo--></td>"
response.write "</tr>"
response.write "</table>"
...
response.write "<tr>"
response.write "	<td class="campop" align="left"><b>Período " & turnod & "</td>"
...
response.write "	<td class=fundo align="center" rowspan=1>Faltas</td>"
...
if lastperlet<>rs("perlet")&rs("turno")&rs("coddoc") then 'inicio cabecalho
	if inicio=0 then
%>
<tr>
...
<%for a=1 to 2 '14 to rs.fields.count-1 %>	
	<td class=campo colspan=3 align="center">&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;</td>
<%next%>
	<td class=campo colspan=2>&nbsp;</td>
</tr>
<%
	linha=linha+1
	end if 'ja iniciou impressao
...
%>
<tr>
	<td class=campo colspan=3 height=<%=altura%>><b><%=rs("curso")%></td>
<%
rsc.movefirst
...
rsc.close
%>
	<td class="campor" colspan=2 height=<%=altura%>>Período Letivo: <%=rs("perlet")%> - <%=turnod%></td>
</tr>
<%
linha=linha+1
end if 'fim cabecalho
...
<%end if%>
	<td class=campo>&nbsp;</td>
	<td class=campo>&nbsp;<%=rs("codtur")%></td>
</tr>
<%
linha=linha+1
inicio=0
lastperlet=rs("perlet")&rs("turno")&rs("coddoc")
...
%>
<tr>
	<td class=campo colspan=3 height=<%=altura%>><b>ORIENTAÇÕES</td>
<%
response.write "<td colspan=3 align="center" style='font-size:7pt; font-family:tahoma;font-weight:normal;background-color:Silver;color:Black;'>Inicio</td>"
response.write "<td colspan=3 align="center" style='font-size:7pt; font-family:tahoma;font-weight:normal;background-color:Silver;color:Black;'>Termino</td>"
linha=linha+1
%>
...
<table border="0" cellpadding="3" cellspacing="0" style="border-collapse: collapse" width="<%=tamanho%>">
<tr>
	<td class="campop" align="left"><b>FOLHA DE PRESENÇA DOS CURSOS DA PÓS-GRADUAÇÃO</b></td>
	<td class="campop" align="right"><b><font size=5><%=pagina%></b></td>
</tr>
<tr>
	<td class="campop" align="left"><%=formatdatetime(dataponto,1)%></td>
...
<tr>
	...
	<td class=titulo align="center" rowspan=1>Nome / Disciplina</td>
	<td class=titulo align="center" rowspan=1>Assinatura</td>
	<td class=titulo align="center" colspan=6>Classes</td>
	<td class=fundo align="center" rowspan=1>Faltas</td>
	<td class=fundo align="center" rowspan=1>Observações</td>
</tr>
...
<%FOR B=1 TO 3%>
<tr>
	<td class="campor" WIDTH=30 height=35>&nbsp;</td>
	<td class="campor" WIDTH=250 nowrap><b>&nbsp;</b><br>&nbsp;</td>
	<td class="campop" width=150>&nbsp;</td>
<%for a=1 to 6 '14 to rs.fields.count-1 %>	
	<td class=campo WIDTH=15 align="center">&nbsp;</td>
<%next%>
	<td class=campo colspan=2>&nbsp;</td>
</tr>
<%NEXT%>
