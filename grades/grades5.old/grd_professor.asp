...
<%
sqla="SELECT F.CHAPA, F.NOME FROM grades_5ch g INNER JOIN " & _
"grades_aux_prof as F ON g.chapa1 = F.CHAPA " & _
"WHERE f.chapa>'00000' " & _
...
rs.close
'p1=year(now)
'if month(now)<=7 then p2=1 else p2=2
'p0=p1&"/"&p2
'if request.form("chapa")="" then response.write "<option value='"&p0&"'>"&p0&"</option>"
%>  
	</select>
...
<tr>
	<td class="campol" colspan="<%=1+ubound(diasemana)+1%>">Grade Horária em <%=formatdatetime(now,2)%></td>
</tr>
<tr>
...
<%
			'for c=0 to ubound(diasemana)
				numero=0
				sql4="SELECT descricao FROM grades_5ch WHERE deletada=0 " & _
				"AND perlet2='" & perlet2 & "' AND chapa1='" & chapa & "' " & _
...
'linha=linha+ubound(horaini)
				for d=0 to ubound(horaini)
					...
					for e=0 to ubound(diasemana)
						...
						horini=horaini(d)
						'horfim=horafim(d)
						sql6="SELECT perlet, coddoc, curso, codtur, codmat, materia, serie, turma, chapa1, nome, codsala, juntar, jturma, dividir, inicio, termino FROM grades_5ch, " & _
						"grades_aux_prof as pfunc WHERE deletada=0 AND chapa1=chapa " & _
						" AND perlet2='" & perlet2 & "' AND diasem=" & diasem & _
						" AND descricao='" & horini & "' and chapa='" & chapa & "' order by inicio "
						rs.Open sql6, ,adOpenStatic, adLockReadOnly
						celula1=""
						if rs.recordcount>0 then
							do while not rs.eof
							codcur=rs("coddoc"):curso=rs("curso")
							codmat=rs("codmat"):materia=rs("materia")
							serie=rs("serie")
							turma=rs("turma")
							chapa1=rs("chapa1")
							nome=rs("nome")
...
'	end if
	...
	compl(d,e)=alunos & obs
							celula1=celula1 & "<b>" & curso & " / " & serie & turma & "</b><br>" & materia & " (entre " & inicio & " e " & termino & ")" & "<br>Sala " & sala
