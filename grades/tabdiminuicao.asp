...
if finaliza=0 then
%>
<p class=titulo>Seleção para impressão de Checagem de atribuição de carga horária - diminuição</p>
<form method="POST" action="tabdiminuicao.asp" name="form">
...
<%
sqla="SELECT c.coddoc, c.curso from g2cursoeve c, g2turmas t where t.coddoc=c.coddoc and c.coddoc in (select coddoc from grades_user where usuario='" & session("usuariomaster") & "') and perlet like '2016%' GROUP BY c.coddoc, c.curso order by c.curso "
...
rs.close
%>  
	<option value="TADM">Administração/Marketing</option>
	<option value="TENG">Todas Engenharias</option>
	</select></td></tr>
</table>
...
periodo1="20161":periodo2="20162"
dtfinal="07/31/2016"
dtinicio="08/01/2016"
	coddoc2=coddoc
	if coddoc="TENG" then coddoc2="EGA','EGC','EGP','EGT"
	if coddoc="TADM" then coddoc2="AMK','AEM"
	...
	"WHERE t.chapa1=f.CHAPA collate database_default AND t.coddoc in ('" & coddoc2 & "') AND f.CODSITUACAO In ('A','Z','F') " & _
...
%>
<table border="1" bordercolor="#000000" cellpadding="1" cellspacing="0" style="border-collapse: collapse" width=1000>
<tr><td class=grupo colspan=9>Atribuição de Aulas - Checagem de diminuição de carga horária</td></tr>
...
<%
mensagem="<br>Curso,Chapa,Nome,Anterior,Atual"
...
if rs("ant")="" then r1="0" else r1=rs("ant")
if rs("atual")="" then r2="0" else r2=rs("atual")
mensagem=mensagem & "<br>" & rs("curso") & "," & rs("chapa1") & "," & rs("nome") & "," & r1 & "," & r2
%>	
<tr>
...
"where chapa1='" & rs("chapa1") & "' and coddoc in ('" & coddoc2 & "') and '" & dtfinal & "' between inicio and termino and deletada=0 and ativo=1 " & _
"group by codtur, turno, serie, turma, codmat, materia "
'response.write sqlsem1
...
<%
sqlsem1="select codtur, turno, serie, turma, codmat, materia, sum(ta) as aulas from g2ch g " & _
"where chapa1='" & rs("chapa1") & "' and coddoc in ('" & coddoc2 & "') and '" & dtinicio & "' between inicio and termino and deletada=0 and ativo=1 " & _
"group by codtur, turno, serie, turma, codmat, materia "
rs2.Open sqlsem1, ,adOpenStatic, adLockReadOnly
numero2=0
if rs2.recordcount>0 then
...
<%
maximo=0
if isnull(checagem) then checagem=0
...
for a=0 to ubound(diminuida)
	if diminuida(a)<>"" then
		...
		"where coddoc in ('" & coddoc2 & "') and left(codtur,7)='" & left(turma(a),7) & "' and codmat='" & materia(a) & "' and '" & dtinicio & "' between inicio and termino and f.chapa collate database_default=g.chapa1  and deletada=0 and ativo=1 " & _
...
for a=0 to ubound(aumentada)
	if aumentada(a)<>"" then
		sqlsupri="select chapa1, f.nome, materia, codtur, codmat, f.codsituacao, sum(ta) as aulas from g2ch g, corporerm.dbo.pfunc f " & _
		"where coddoc in ('" & codcur2 & "') and left(codtur,7)='" & left(turma2(a),7) & "' and codmat='" & materia2(a) & "' and '" & dtfinal & "' between inicio and termino and f.chapa collate database_default=g.chapa1  and deletada=0 and ativo=1 " & _
		"group by chapa1, f.nome, materia, codtur, codmat, f.codsituacao "
		rs2.Open sqlsupri, ,adOpenStatic, adLockReadOnly
		if rs2.recordcount>0 then
			rs2.movefirst
...
%>
<tr>
	<td class=titulo colspan=9>Não houve diminuição de carga horária dos professores</td>
</tr>
