...
<form method="POST" action="crachaaluno.asp" name="form">
...
<%
'sql2="select matricula, nome from ealunos order by nome " 
'sql2="select e.matricula, e.nome, e.dtnasc, e.idimagem, c.codcur, c1.nome as curso " & _
'"from ealunos e, ualucurso c, ucursos c1, umatricpl pl " & _
'"where e.matricula=c.mataluno and c.codcur=c1.codcur and pl.mataluno=c.mataluno and pl.codcur=c.codcur " & _
'"and pl.perletivo like '2007%' "
'rs2.Open sql2, ,adOpenStatic, adLockReadOnly
'response.write "<option value='0'>Selecione Aluno....</option>"
'rs2.movefirst:do while not rs2.eof
'if chapa=rs2("matricula") then tempc="selected" else tempc=""
'	response.write "<option value='" & rs2("matricula") & "' " & tempc & "><" & rs2("nome") & "></option>"
'rs2.movenext:loop
'rs2.close
%>
	</select> -->
...
sql1="select e.matricula, e.nome, e.dtnasc, e.idimagem, c.codcur, c1.nome as curso " & _
"from corporerm.dbo.ealunos e, corporerm.dbo.ualucurso c, corporerm.dbo.ucursos c1, corporerm.dbo.umatricpl pl " & _
"where e.matricula='" & chapa & "' and e.matricula=c.mataluno and c.codcur=c1.codcur and pl.mataluno=c.mataluno and pl.codcur=c.codcur " 
'"and pl.perletivo like '2007%' "
rs2.Open sql1, ,adOpenStatic, adLockReadOnly
%>
<div align="center">
<!--
<table width="640" height="200" border=1 cellspacing=0 cellpadding=0 style='border-collapse: collapse' bordercolor="#dcdcdc">
<tr><td valign="top" align="center" width=320>
-->
...
<%
'Response.Write getCodigoBarras(chapa) & "<br>"
'response.write (chapa) & "<br>"
'Response.Write getCodigoBarras(chapa & digito(chapa)) & "<br>"
'response.write chapa & digito(chapa) & "<br>"
chapaCB=request.form("via")&chapa
Response.Write getCodigoBarras(chapa&request.form("via")) & "<br>"
'response.write chapa & digito(chapa) & "<br>"
'Response.Write getCodigoBarras(chapa) & "<br>"
'response.write chapa & "<br>"
