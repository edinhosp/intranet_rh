...
<tr><td class=titulo>Chapa</td><td class=titulo>Mês</td><td class=titulo>Ano</td></tr>
<tr>
...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" width="600">
<tr><td class=campo>Período: <%=mes & "/" & ano %></td></tr></table>
<hr>
...
dim marc(31,6)
dim formato(31,6)
	sqlcr="select * from (select b.chapa, day(data) as dia, data, batida, natureza, status, justificativa from corporerm.dbo.abatfunam b left join corporerm.dbo.AAFDT f on f.ID=b.IDAAFDT and f.CHAPA=b.CHAPA where b.chapa='" & chapa & "' and month(data)=" & mes & " and year(data)=" & ano & " " & _
	"UNION ALL " & _
	"select b.chapa, day(data) as dia, data, batida, natureza, status, justificativa from corporerm.dbo.abatfun b left join corporerm.dbo.AAFDT f on f.ID=b.IDAAFDT and f.CHAPA=b.CHAPA where b.chapa='" & chapa & "' and month(data)=" & mes & " and year(data)=" & ano & " " & _
	") a order by data, batida "
	...
	if rs2.recordcount>0 then
		...
		marc(dia,marcacao)=batida
		if rs2("status")="D" and rs2("justificativa")<>"Marcação Teste" then formato(dia,marcacao)="<font color='red'>" 'else formato(dia,marcacao)="<font color='black'>"
		diaant=dia
