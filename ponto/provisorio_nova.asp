...
<title>Inclusão de Entrega de Provisório</title>
...
'if request.form("bolsa")="ON" then bolsa=-1 else bolsa=0
		...
		sqlb = " SELECT 'E','" & request.form("provisorio") & "'" & _
...
'if request.form("bt_salvar")="" then
%>
<form method="POST" action="provisorio_nova.asp" name="form">
...
<table border="0" cellpadding="3" cellspacing="0" style="border-collapse: collapse" width="465">
	<tr><td class=grupo>Inclusão de Entrega de Provisório</td></tr>
</table>
<!-- tipo lancamento -->
<table border="0" cellpadding="3" cellspacing="0" style="border-collapse: collapse" width="465">
<tr>
...
<%
sqla="select a.codcracha from corporerm.dbo.acracha a where a.situacao=1 and a.codcracha not in ( " & _
...
if request.form("provisorio")=rsd("codcracha") then tempc="selected" else tempc=""
%>
...
%>
	...
	<td class=fundo><input type="text" value="<%=chapa%>" name="chapa" size="5" onfocus="javascript:window.status='Informe o chapa do funcionário'" onchange="chapa1()">
...
rsd.movefirst:do while not rsd.eof
if chapa=rsd("chapa") then temp="selected" else temp=""
%>
